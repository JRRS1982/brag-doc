...
* **Enabling non-engineers to build and deploy their own features**
  Historically, every AB test on `chameleon-results` (the full page app/iframe overlay on top of the chameleon thank you page) required engineering time to build, review and deploy, which bottlenecked the Growth and Design teams and meant slow iteration on up-sell experiments.
  I suggested and have been leading a project to enable non-engineers to build, deploy and AB test their own versions of `chameleon-results` using AI tooling such as claude code, in staging and production, without engineering involvement. Working with the platform team, this involved ephemeral ECS instances of `chameleon-results` deployed on separate domains (using a wildcard with the branch as the subdomain) via a CircleCi pipeline, and a feature flag in the main `chameleon` service that lets non-engineers select the version of `chameleon-results` they created. Guard rails and automated testing ensure features are safe to deploy, master is protected, and the platform stays stable.
  Non-engineers can now ship and AB test against production traffic without pulling engineers into reviews, which protects engineering time and dramatically shortens the AB test cycle. This may save multiple weeks or make it possible to test ideas which would never have been prioritised. The Director of Growth and the Design team have expressed great appreciation, noting this will be incredibly useful to them going forward and it is already being used.

* **10% time – Replacing flaky Selenium tests with Playwright**
  The team has long been frustrated by a custom Selenium test suite (`chameleon-acceptance-tests`). It can take 1 hour 45 minutes to run, is full of race conditions and flaky behaviour, and has severely impacted developer productivity and engagement. As a result, we are missing coverage for key features.
  I chose to dedicate my 10% study time to building a prototype replacement: a new `chameleon-tests` service using Playwright. Initial results show Playwright is vastly superior to Selenium and integrates cleanly with BrowserStack. An equivalent single test now runs in ~236ms instead of ~4 minutes, and the sanity tests run in 1min 45s. Given there are hundreds of tests run multiple times per day across multiple suites, this is a massive improvement in speed, reliability, and developer experience.
  The project was presented to the tech department for feedback and endorsement, received positive feedback, and has since been rolled out to the wider team. The sanity tests now run with chameleon (where there was no coverage before) and we continue to build out the new test suite alongside the existing Selenium suite until coverage matches, at which point we can retire the old suite. This will save significant engineering time and improve test reliability.
